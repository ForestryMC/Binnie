...
public class WindowFieldKit extends Window {
	...
	public void initialiseServer() {
		//create slots
		WindowInventory inventory = this.getWindowInventory();
		ItemStack kit = this.getPlayer().getHeldItemMainhand();
	...
	public void onUpdateClient() {
		...
		if (this.isAnalysing) {
			...
			if (this.analyseProgress >= 1) {
				this.isAnalysing = false;
				this.analyseProgress = 1;
				ItemStack stack = this.getWindowInventory().getStackInSlot(INDIVIDUAL_SLOT);
				if (!stack.isEmpty()) {
	...
	private void refreshSpecies() {
		ItemStack item = this.getWindowInventory().getStackInSlot(INDIVIDUAL_SLOT);
	...
	public void onWindowInventoryChanged() {
		...
		if (this.isServer()) {
			...
			int sheets = 64 - kit.getItemDamage();
			int size = (paper.isEmpty()) ? 0 : paper.getCount();
			if (sheets != size) {
		...
		if (this.isClient()) {
			ItemStack item = inventory.getStackInSlot(INDIVIDUAL_SLOT);
			this.text.setValue("");
