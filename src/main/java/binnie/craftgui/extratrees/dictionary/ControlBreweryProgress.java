...
public class ControlBreweryProgress extends ControlProgressBase {
	...
	public void onRenderBackground(int guiWidth, int guiHeight) {
		...
		CraftGUI.render.fluid(new IPoint(1, 6), logic.currentCrafting.inputFluid);
		CraftGUI.render.fluid(new IPoint(17, 6), logic.currentCrafting.inputFluid);
		CraftGUI.render.fluid(new IPoint(1, 22), logic.currentCrafting.inputFluid);
		CraftGUI.render.fluid(new IPoint(17, 22), logic.currentCrafting.inputFluid);
		...
		GL11.glEnable(GL11.GL_SCISSOR_TEST);
		CraftGUI.render.fluid(new IPoint(1, 6), BreweryRecipes.getOutput(logic.currentCrafting));
		CraftGUI.render.fluid(new IPoint(17, 6), BreweryRecipes.getOutput(logic.currentCrafting));
		CraftGUI.render.fluid(new IPoint(1, 22), BreweryRecipes.getOutput(logic.currentCrafting));
		CraftGUI.render.fluid(new IPoint(17, 22), BreweryRecipes.getOutput(logic.currentCrafting));
		GL11.glDisable(GL11.GL_SCISSOR_TEST);
		...
		for (final ItemStack stack : stacks) {
			CraftGUIUtil.renderItem(new IPoint(x, y), stack);
			x += 16;
			if (x > 18) {
				x = 1;
				y += 16;
