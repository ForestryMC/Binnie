...
public enum ExtraBeesEffect implements IAlleleBeeEffect {
	...
	private void onEctoplasmEffect(World world, int x, int y, int z) {
		...
		if (world.isAirBlock(x, y, z) && (world.isBlockNormalCubeDefault(x, y - 1, z, false) || world.getBlock(x, y - 1, z) == ExtraBees.ectoplasm)) {
	...
	public <T extends Entity> List<T> getEntities(Class<T> eClass, IBeeGenome genome, IBeeHousing housing) {
		...
		int[] min = {housing.getCoordinates().posX + offset[0], housing.getCoordinates().posY + offset[1], housing.getCoordinates().posZ + offset[2]};
		int[] max = {housing.getCoordinates().posX + offset[0] + area[0], housing.getCoordinates().posY + offset[1] + area[1], housing.getCoordinates().posZ + offset[2] + area[2]};
