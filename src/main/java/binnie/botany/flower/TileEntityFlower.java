...
public class TileEntityFlower extends TileEntity implements IPollinatable, IButterflyNursery {
	...
	public Packet getDescriptionPacket() {
		...
		return (renderInfo != null) ?
			Botany.instance.getNetworkWrapper().getPacketFrom(new MessageFlowerUpdate(xCoord, yCoord, zCoord, renderInfo).getMessage()) :
			null;
	}
	...
	public void spawnButterflies() {
		...
		IButterfly spawn = Binnie.Genetics.getButterflyRoot().getIndividualTemplates().get(worldObj.rand.nextInt(Binnie.Genetics.getButterflyRoot().getIndividualTemplates().size()));
	...
	private void matureCaterpillar() {
		...
		matureTime++;
		if (matureTime >= caterpillar.getGenome().getLifespan() / (caterpillar.getGenome().getFertility() * 2) && caterpillar.canTakeFlight(worldObj, xCoord, yCoord, zCoord)) {
