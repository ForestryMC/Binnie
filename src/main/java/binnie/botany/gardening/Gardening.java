...
public class Gardening implements IGardeningManager {
	...
	@Override
	public boolean isSoil(final Block block) {
		return block instanceof IBlockSoil;
	}
	
	@Override
	public boolean isSoil(final Item item) {
		return item instanceof ItemBlock && isSoil(((ItemBlock) item).getBlock());
	}
	
	@Override
	public boolean isSoil(final ItemStack item) {
	...
	@Override
	public void plantSoil(final World world, final BlockPos pos, final EnumSoilType soil, final EnumMoisture moisture, final EnumAcidity acidity) {
	...
	@Override
	public boolean isAcidFertiliser(final ItemStack itemstack) {
	...
	@Override
	public boolean isAlkalineFertiliser(final ItemStack itemstack) {
	...
	@Override
	public boolean isNutrientFertiliser(final ItemStack itemstack) {
	...
	@Override
	public int getFertiliserStrength(final ItemStack stack) {
		for (final Map.Entry<ItemStack, Integer> entry : fertiliserAcid.entrySet()) {
			if (entry.getKey().isItemEqual(stack)) {
				return entry.getValue();
			}
		}
		for (final Map.Entry<ItemStack, Integer> entry : fertiliserAlkaline.entrySet()) {
			if (entry.getKey().isItemEqual(stack)) {
				return entry.getValue();
			}
		}
		for (final Map.Entry<ItemStack, Integer> entry : fertiliserNutrient.entrySet()) {
			if (entry.getKey().isItemEqual(stack)) {
				return entry.getValue();
			}
		}
	...
	@Override
	public boolean canTolerate(@Nullable final IFlower flower, final World world, final BlockPos pos) {
		if (flower == null) {
			return false;
		}
	...
	@Override
	public EnumSoilType getSoilType(final World world, final BlockPos pos) {
	...
	@Override
	public Block getSoilBlock(final EnumSoilType type) {
