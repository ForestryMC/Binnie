...
public class AnalystPageProducts extends AnalystPageProduce {
	public AnalystPageProducts(IWidget parent, IArea area, IBee ind) {
		...
		y += 12;
		String alleleName = Binnie.Genetics.beeBreedingSystem.getAlleleName(EnumBeeChromosome.SPEED, ind.getGenome().getActiveAllele(EnumBeeChromosome.SPEED));
		new ControlTextCentered(this, y, EnumChatFormatting.ITALIC + I18N.localise("genetics.gui.analyst.produce.rate", alleleName))
			.setColor(getColor());
		...
		if (!products.isEmpty()) {
			new ControlTextCentered(this, y, I18N.localise("genetics.gui.analyst.produce.speciality")).setColor(getColor());
			y += 12;
			for (Map.Entry<ItemStack, Float> entry : products.entrySet()) {
				refinedProducts.addAll(getAllProducts(entry.getKey()));
				createProductEntry(entry.getKey(), entry.getValue(), y, speed * modeSpeed);
		...
		new ControlTextCentered(this, y, I18N.localise("genetics.gui.analyst.produce.refined"))
			.setColor(getColor());
