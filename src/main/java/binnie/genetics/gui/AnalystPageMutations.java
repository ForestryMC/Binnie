	...
	public AnalystPageMutations(IWidget parent, IArea area, IIndividual ind, boolean isMaster) {
		...
		if (ind instanceof IBee) {
			...
			if (ind.getGenome().getPrimary() == BeeDefinition.VALIANT.getGenome().getPrimary()) {
				...
				y += 10;
				new ControlTextCentered(this, y, EnumChatFormatting.ITALIC + I18N.localise("genetics.gui.analyst.mutations.habitat.any"))
					.setColor(getColor());
				y += 22;
				...
				y += 10;
				new ControlTextCentered(this, y, EnumChatFormatting.ITALIC + I18N.localise("genetics.gui.analyst.mutations.habitat.villagers"))
					.setColor(getColor());
				y += 22;
			} else if (hive != null) {
				new ControlTextCentered(this, y, I18N.localise("genetics.gui.analyst.mutations.habitat"))
				...
				if (!isMaster && !isKnown(system, mutation)) {
					...
					new Control(this, ox + dx, y, 44.0f, 16.0f) {
						...
						public void initialise() {
							...
							addTooltip(comb);
							String chance = getMutationColor(mutation.getBaseChance()).getCode() + I18N.localise("genetics.gui.analyst.mutations.chance", (int) mutation.getBaseChance());
							if (specificChance != mutation.getBaseChance()) {
								chance += getMutationColor(specificChance).getCode() + " " + I18N.localise("genetics.gui.analyst.mutations.chance", (int) specificChance);
							}
					...
					new Control(this, ox + dx, y, 44.0f, 16.0f) {
						...
						public void initialise() {
							...
							if (specificChance2 != mutation.getBaseChance()) {
								chance = chance + getMutationColor(specificChance2).getCode() + " " + I18N.localise("genetics.gui.analyst.mutations.chance", (int) specificChance2);
							}
							addTooltip(chance);
							for (String s : mutation.getSpecialConditions()) {
								addTooltip(s);
