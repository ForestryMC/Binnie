...
public class DoorBlockRenderer implements ISimpleBlockRenderingHandler {
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
		...
		tessellator.setBrightness((renderer.renderMinY > 0.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y - 1, z));
		...
		tessellator.setBrightness((renderer.renderMaxY < 1.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y + 1, z));
		...
		tessellator.setBrightness((renderer.renderMinZ > 0.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y, z - 1));
		...
		tessellator.setBrightness((renderer.renderMaxZ < 1.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x, y, z + 1));
		tessellator.setColorOpaque_F(f3 * c2, f3 * c3, f3 * c4);
		icon = renderer.getBlockIcon(block, renderer.blockAccess, x, y, z, 3);
		renderer.renderFaceZPos(block, x, y, z, icon);

		renderer.flipTexture = false;
		tessellator.setBrightness((renderer.renderMinX > 0.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x - 1, y, z));
		tessellator.setColorOpaque_F(f4 * c2, f4 * c3, f4 * c4);
		icon = renderer.getBlockIcon(block, renderer.blockAccess, x, y, z, 4);
		renderer.renderFaceXNeg(block, x, y, z, icon);

		renderer.flipTexture = false;
		tessellator.setBrightness((renderer.renderMaxX < 1.0) ? i1 : block.getMixedBrightnessForBlock(renderer.blockAccess, x + 1, y, z));
		tessellator.setColorOpaque_F(f4 * c2, f4 * c3, f4 * c4);
