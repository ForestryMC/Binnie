...
public class ModuleGenetics implements IInitializable {
	...
	public void postInit() {
		...
		for (ButterflySpecies species2 : ButterflySpecies.values()) {
			...
			String scientific = species2.branchName.substring(0, 1).toUpperCase() + species2.branchName.substring(1).toLowerCase();
	...
	private void generateBranches() {
		...
		for (ExtraTreeSpecies species : ExtraTreeSpecies.values()) {
			IClassification branch = getOrCreateClassification(IClassification.EnumClassLevel.GENUS, species.branchName);
			...
			while (clss.getParent() == null) {
				for (List<String> set2 : classifications) {
					if (set2.contains(clss.getScientific().toLowerCase())) {
						...
						IClassification parent = getOrCreateClassification(IClassification.EnumClassLevel.values()[currentLevel], nextLevel);
